...
describe("Sidebar", () => {
	...
	it("opens the create-project flow when the no-project shortcut signal arrives", async () => {
		...
		expect(await screen.findByRole("dialog", { name: "Import to AICodeRoom" })).toBeInTheDocument();
	});
	...
	it("defaults worker and orchestrator agents when creating a project", async () => {
		...
		await user.click(screen.getByLabelText("New project"));
		expect(screen.getByRole("dialog", { name: "Import to AICodeRoom" })).toBeInTheDocument();
	...
	it("shows ancestor repo warning in agent sheet for workspace inside existing repo", async () => {
		...
		expect(
			screen.getByText(
					"If this folder needs Git setup, AICodeRoom will initialize it and create the first commit before starting.",
			),
		).toBeInTheDocument();
		await chooseOption(screen.getByRole("combobox", { name: "Orchestrator agent" }), "Claude Code");
		await user.click(screen.getByRole("button", { name: "Create workspace and start" }));
